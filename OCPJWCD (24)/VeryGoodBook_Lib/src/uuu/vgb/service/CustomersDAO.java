...
public class CustomersDAO {//Data Access Object
    ...
    public void insert(Customer c) throws VGBException {
        try (
            ...
            if (ex.getErrorCode() == 1062) {
                throw new VGBException("新增客戶失敗-身分證號或email已重複註冊", ex);
            }
    ...
    private static final String SELECT_ALL = "SELECT id, name,password,gender,email,birthday,"
            + "address,phone,married,blood_type,discount,type "
            + "FROM customers ";

    public List<Customer> selectAll() throws VGBException {
        List<Customer> list = new ArrayList<>();
        //TODO: customers資料表格中所有資料讀出來，一筆資料存入一個Customer物件，將Customer物件加入list中
        try (//2.取得連線
                Connection connection = RDBConnection.getConnection();
                //3. 準備指令
