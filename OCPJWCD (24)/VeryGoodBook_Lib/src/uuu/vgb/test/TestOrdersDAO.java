...
public class TestOrdersDAO {
    public static void main(String[] args) {
        try {
            ...
            Customer member = cService.login("A123456789", "123456");//VIP
            Customer member2 = cService.login("A123123123", "123456");//一般客戶
            ...
            order.setReceiverPhone(
                order.getMember().getPhone()!=null && order.getMember().getPhone().length()>0
                        ?order.getMember().getPhone():"25142222");
            order.setShippingAddr(
                order.getMember().getAddress()!=null && order.getMember().getAddress().length()>0?order.getMember().getAddress():"台北市復興北路99號2F");
