...
public class CaptchaImageServlet extends HttpServlet {
    ...
    public CaptchaImageServlet() {
        System.out.println("CaptchaImageServlet 建立了一個物件");        
    }    
...
private BufferedImage generateImage(String rand, int width, int height) {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (rand == null) {
            ...
            HttpSession session = request.getSession(); //第15章session介紹
            session.setAttribute(this.getServletName(), rand); //(之後必須加回這行code) , getServletName()
        }
