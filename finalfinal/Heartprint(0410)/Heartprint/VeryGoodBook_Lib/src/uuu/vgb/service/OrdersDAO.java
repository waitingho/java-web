...
public class OrdersDAO {
    ...
    public void inster(Order order) throws VGBException {
        if (order == null) {
        ...
        try (Connection connection = RDBConnection.getConnection();//2. 建立連線
            ...
            for (OrderItem item : order.getOrderItemSet()) {
                int rows;
                //3.1傳入指令01的?值
                pstmt01.setInt(1, item.getQuantity());
                pstmt01.setInt(2, item.getProduct().getId());
                pstmt01.setInt(3, item.getQuantity());
                //4.執行
                rows = pstmt01.executeUpdate();
                if (rows == 0) {
                    throw new VGBException("產品庫存不足- " + item.getProduct().getId());
                }
            }
...
//                    item.setColor(rs.getString("color"));
     ...
     private static final String SELECT_ORDER_STATUS_LOG_BY_ID = "SELECT order_id,"
            + "old_status,new_status,update_time "
            + "FROM order_status_log WHERE order_id=?";

    public List<OrderStatusLog> selectOrderStatusLogById(int id) throws VGBException {
        List<OrderStatusLog> list = new ArrayList<>();
        try (Connection connection = RDBConnection.getConnection(); //2. 建立連線
                PreparedStatement pstmt = connection.prepareStatement(SELECT_ORDER_STATUS_LOG_BY_ID) //3. 準備指令
                ) {
            //3.1 傳入?的值
            pstmt.setInt(1, id);
            ...
            try(ResultSet rs = pstmt.executeQuery()){
                while(rs.next()){
                    ...
                    log.setUpdateTime(rs.getString("update_time"));
                    list.add(log);
                }
            }            
            return list;
        } catch (SQLException ex) {
            System.out.println("查詢訂單狀態失敗:" + ex);
            throw new VGBException("查詢訂單狀態失敗!", ex);
        }
    }
