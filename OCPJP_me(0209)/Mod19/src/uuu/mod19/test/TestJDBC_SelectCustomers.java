...
public class TestJDBC_SelectCustomers {
    ...
    public static void main(String[] args) {
        String sql = "SELECT id,name,password,gender,email,birthday,"
                + "address,phone,married,blood_type,discount,type FROM customers";
        try {
            //1.載入JDBC driver
            ...
            try (
                ...
                //5.處理resultset
                List<Customer> customersList = new ArrayList<>(5);
                while(rs.next()){
...
//                    Customer c = new Customer();
                    try{
                        ...
                        customersList.add(0,c);
                    }catch(VGBException ex){
                        System.out.println(ex);
                    }
                }
                System.out.println("customersList:" + customersList);
                System.out.println("customersList:" + customersList.size());
            } catch (SQLException ex) {
                Logger.getLogger(TestJDBC_SelectCustomers.class.getName()).log(Level.SEVERE, "無法建立連線", ex);
            }
            
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(TestJDBC_SelectCustomers.class.getName()).log(Level.SEVERE, "無法載入JDBC Driver", ex);
        }
    }
}
